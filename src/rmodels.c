...
static Model LoadOBJfromString(const char *fileText);     // Load OBJ mesh data from string
#endif
...
Model LoadModelFromObjString(const char *fileText)
{
    Model model = { 0 };

#if defined(SUPPORT_FILEFORMAT_OBJ)
    model = LoadOBJfromString(fileText);
#endif
    ...
    if (model.meshCount == 0)
    {
        ...
        model.meshes = (Mesh *)RL_CALLOC(model.meshCount, sizeof(Mesh));
#if defined(SUPPORT_MESH_GENERATION)
        TRACELOG(LOG_WARNING, "MESH: [ObjString] Failed to load mesh data, default to cube mesh");
        model.meshes[0] = GenMeshCube(1.0f, 1.0f, 1.0f);
#else
        TRACELOG(LOG_WARNING, "MESH: [ObjString] Failed to load mesh data");
#endif
    }
    else
    {
    ...
    if (model.materialCount == 0)
    {
        TRACELOG(LOG_WARNING, "MATERIAL: [ObjString] Failed to load material data, default to white material");
...
static Model LoadOBJfromString(const char *fileText)
{
    Model model = { 0 };
    ...
    if (fileText != NULL)
    {
        ...
        if (ret != TINYOBJ_SUCCESS) TRACELOG(LOG_WARNING, "MODEL: [ObjString] Failed to load OBJ data");
        else TRACELOG(LOG_INFO, "MODEL: [ObjString] OBJ data loaded successfully: %i meshes/%i materials", meshCount, materialCount);
